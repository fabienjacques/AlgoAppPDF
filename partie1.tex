...
%Proposition de texte : Le problème ici n'est plus seulement un problème de décision mais aussi d'optimisation. Nos défenseurs ont des coordonnées d'origine (positions initiale) et on doit trouver une solution du problème qui minimise la mise en place des défenseurs (minimiser le trajet de la position initiale vers position d'interception de tir). Dans notre cas, parmi toutes les solutions d'un problème (ensemble d'ensemble dominants), on veut sélectionner celui qui minimise la distance du défenseur le plus éloigné de sa position d'interception. En outre, on choisi la solution qui met en place tous les défenseurs le plus vite possible.
